...
String changelog(String body) => '# Changelog\n\n## 9.0.0 — 2026-09-13\n\n- An entry.\n\n$body\n';

void main() {
  group('refuses', () {
    const refused = <String, String>{
      'a tag': '<meta http-equiv="refresh" content="0;url=https://evil.example">',
      'a tag split over lines': '<style\ntype="text/css">.changelog-body{display:none}</style\n>',
      'a tag left unterminated': '<div style="position:fixed;inset:0;background:red"',
      'a fence shadowed by a tag': '<div\n```\n<meta http-equiv="refresh" content="0;url=https://e.example/">\n```',
      ...
      'a javascript: link written with an entity': '- [notes](&#106;avascript:alert(1))',
      'a javascript: link whose entity has no semicolon': '- [notes](javascript&#58alert(1))',
      'a javascript: link whose entity is hexadecimal': '- [notes](javascript&#x3A+alert(1))',
      'a javascript: link written with a named entity': '- [notes](javascript&colon;alert(1))',
      'a javascript: link in an angle-bracketed destination': '- [notes](<&#106;avascript:alert(1)>)',
      'a javascript: link through a reference': '- [notes][r]\n\n[r]: &#106;avascript:alert(1)',
      'a javascript: autolink': '- <javascript:alert(1)>',
      'a scheme-relative link': '- [notes](//evil.example/path)',
      'a scheme-relative link spelled with backslashes': '<a href="\\\\evil.example/path">notes</a>',
      'a link whose slash hides a tab': '<a href="/\t/evil.example/path">notes</a>',
      ...
      'a link whose control character is written as an entity': '<a href="&#1;//evil.example/">notes</a>',
      ...
      'an image whose path hides a tab': '<img src="/\t/evil.example/p.png" alt="x">',
      'a link whose tab is written as an entity': '<a href="/&#9;/evil.example/">notes</a>',
      'a link whose newline is written as an entity': '<a href="/&#10;/evil.example/">notes</a>',
      'an attribute written without a value': '<input type="checkbox" disabled>',
      'a class the page does not write': '<ul class="nav"><li>x</li></ul>',
      'an alignment that is not one': '<table><tr><td align="javascript:">x</td></tr></table>',
      'a placeholder written as a tag': 'Pass --output <dir> to choose where it goes.',
    };
  ...
  group('renders', () {
    const kept = <String, String>{
      'prose, emphasis and code spans': '- **bold**, _quiet_, `<pad>` and ~~gone~~.',
      'a link': '- [the notes](https://github.com/DenisovAV/litetune/releases)',
      'a link with a title': '- [the notes](https://litetune.dev "the changelog")',
      'a link with a query string': '- [issues](https://github.com/x/y?q=is%3Aopen&sort=created)',
      'a URL with an apostrophe': "- See https://en.wikipedia.org/wiki/Occam's_razor for details.",
      'a link to this site': '- [the entry](/changelog#v9.0.0) and [here](#v9.0.0)',
      ...
      'a fence with a longer info string': '```bash on macOS\nlitetune verify\n```',
    ...
    kept.forEach((what, body) {
      test(what, () {
        expect(() => ChangelogPage.renderBody(changelog(body)), returnsNormally, reason: what);
      });
    });

    test('a table as a table', () {
      final html = ChangelogPage.renderBody(changelog('| a | b |\n| --- | --- |\n| c | d |'));
      expect(html, contains('<table>'));
      expect(html, contains('<td>c</td>'));
    });

    test('a task list as checkboxes', () {
      final html = ChangelogPage.renderBody(changelog('- [x] done\n- [ ] not yet'));
